...
export async function checarRateLimit(supabase, { userId, ip } = {}, endpoint, { limite = 20, janelaMs = 60_000 } = {}) {
  ...
  // Registra esta chamada. Aguarda o insert: em serverless, a função pode ser
  // congelada logo após a resposta e perder um insert sem await.
  const registro = userId ? { user_id: userId, endpoint } : { ip, endpoint }
  const { error: insErr } = await supabase.from('api_rate_limit').insert(registro)
