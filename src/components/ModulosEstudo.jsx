...
const DISC_COR = {
  ...
  "Direito Processual do Trabalho":    "#b45309",
  "Direito Tributário":                "#ea580c",
  "Direito Administrativo":            "#be185d",
  "Direito Empresarial":               "#64748b",
  "Direito Ambiental":                 "#15803d",
  "Direito Eleitoral":                 "#6d28d9",
  "Direito Previdenciário":            "#0369a1",
  "Direito do Consumidor":             "#047857",
  "Direito Internacional":             "#1d4ed8",
  "Direito da Criança e do Adolescente": "#db2777",
  "Direito Financeiro":                "#92400e",
  "Direito Digital e LGPD":            "#4338ca",
  "Direitos Humanos":                  "#0891b2",
  "Filosofia do Direito":              "#78716c",
}
...
function parseBucketPath(storagePath) {
  // "manuais-pdf/Criminologia/Criminologia.pdf" → bucket + path
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          <div
            ...
            <div style={{ position: 'absolute', left: 0, top: 0, height: '100%', width: `${duracao ? (progresso / duracao) * 100 : 0}%`, background: theme.gold, borderRadius: 2 }} />
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: theme.muted, fontFamily: 'IBM Plex Mono, monospace' }}>
            <span>{fmt(progresso)}</span>
...
function MaterialCard({ material, theme }) {
  ...
  const icone = material.tipo === 'manual' ? FileText
               : material.tipo === 'audio' ? Headphones
               : BookOpen
  const Icone = icone

  const cor = material.tipo === 'manual' ? theme.gold
             : material.tipo === 'audio' ? '#10b981'
             : '#a78bfa'

  const label = material.tipo === 'manual' ? 'Manual PDF'
              : material.tipo === 'audio'  ? 'Aula em Áudio'
              : 'Slides PPTX'
  ...
  return (
    <div style={{ border: `1px solid ${theme.border}`, borderRadius: 12, marginBottom: 12, overflow: 'hidden' }}>
      ...
      {/* Conteúdo expandido */}
      {aberto && (
        <div style={{ padding: '12px 16px', background: theme.cardBg || theme.bg, borderTop: `1px solid ${theme.border}` }}>
          {modulo.descricao && (
            <div style={{ fontSize: 12, color: theme.muted, fontFamily: 'Georgia, serif', lineHeight: 1.6, marginBottom: 12, fontStyle: 'italic' }}>
              {modulo.descricao}
            </div>
          )}
          {materiais.map(m => <MaterialCard key={m.id} material={m} theme={theme} />)}
        </div>
      )}
    </div>
  )
...
export default function ModulosEstudo({ theme }) {
  ...
  const disciplinas = ['Todos', ...new Set(modulos.map(m => m.disciplina))]
  const modulosFiltrados = filtro === 'Todos' ? modulos : modulos.filter(m => m.disciplina === filtro)
    ...
    <div>
          ...
          {disciplinas.map(d => {
            ...
            return (
              <button key={d} onClick={() => setFiltro(d)}
                style={{ fontSize: 11, fontFamily: 'IBM Plex Mono, monospace', padding: '4px 10px', borderRadius: 6, cursor: 'pointer', border: `1px solid ${ativo ? cor : theme.border}`, background: ativo ? cor + '18' : 'none', color: ativo ? cor : theme.muted, fontWeight: ativo ? 700 : 400, transition: 'all .15s' }}>
                {d}
              </button>
            )
          })}
      ...
      {modulosFiltrados.map(m => (
        <ModuloCard
          key={m.id}
          modulo={m}
          materiais={materiais.filter(mat => mat.modulo_id === m.id)}
          theme={theme}
        />
      ))}
    </div>
