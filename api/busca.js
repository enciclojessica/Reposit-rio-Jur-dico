...
export default async function handler(req, res) {
  ...
  const { permitido } = await checarRateLimit(supabase, { userId: user.id }, 'busca', { limite: 30, janelaMs: 5 * 60_000 })
  ...
  // Extração de Documentos, que passam todos por aqui.
  const { data: membro } = await supabase
    .from('membros').select('role, pago').eq('user_id', user.id).single()
  const podeUsarIA = membro?.role === 'admin' || !!membro?.pago
  if (!podeUsarIA) {
    return res.status(403).json({ error: 'Este recurso de IA é da versão paga. Peça liberação ao administrador.' })
  }
